...
if ($gclient->getAccessToken()) {
	...
	$sql = mysqli_query($connect, "SELECT idAnggota, nama FROM tbanggota WHERE alamatEmail='".$email."'");
	...
	if(empty($user)){ // Jika User dengan email tersebut belum ada
		...
		mysqli_query($connect, "INSERT INTO tbanggota(tanggal, nama, alamatEmail, verifAkun, idTemp) VALUES('".date('Y-m-d')."','".$nama."', '".$email."', 'Y', '".md5($email)."')");
