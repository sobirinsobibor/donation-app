...
if (isset($_POST['simpan'])) {
 ...
 $unik=substr(rand(0,1234567890),0,3);
    ...
    $query=mysqli_query($con,"INSERT INTO tbberdonasigalangdana(`idTemp`, `idGalangDana`, `kodeUnik`,`idAnggota`, `nama`, `anonim`, `nope`, `email`, `namaUsaha`, `lokasiUsaha`, `idRekening`, `jumlah`, `tanggal`, `tanggalBerakhir`, `status`,`dukungan`) VALUES('".$idTemp."',".$data[0].",'".$no."',Null,'".$nama."','".$namasamar."','".$nope."','".$email."',NULL,NULL,".$bank.",".$hasil.",'".date('Y-m-d h:i:s')."','".date('Y-m-d h:i:s',$besok)."','P','".$dukungan."')");
    if($query){
      echo "
      <script>
